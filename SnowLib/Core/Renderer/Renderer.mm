...
//    pDesc->colorAttachments()->object(0)->t
...
void Renderer::drawSetup( MTKView* view ) {
//    [view setDept]
...
//    _pRpd = MTL::RenderPassDescriptor::alloc()->init();
    _pRpd = (__bridge MTL::RenderPassDescriptor*)[view currentRenderPassDescriptor];
...
void Renderer::initDraw(MTKView* view) {
...
void Renderer::draw( MTKView* view, Node* sceneTree ) {
...
void Renderer::endDraw(MTKView* view) {
    pEnc->endEncoding();
    
    _pCmd->presentDrawable( (__bridge MTL::Drawable*)[view currentDrawable] );
    _pCmd->commit();
